...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    out_one, cache_one = affine_relu_forward(X, W1, b1)
    scores, cache_two = affine_forward(out_one, W2, b2)
    ...
    dx_aff2, grads['W2'], grads['b2'] = affine_backward(dx_soft, cache_two)
    grads['W2'] += self.reg * W2
    _, grads['W1'], grads['b1'] = affine_relu_backward(dx_aff2, cache_one)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers - 1):
        if self.use_batchnorm:
            out, cache = affine_batch_relu_forward(out,self.params["W"+str(i + 1)], self.params["b"+str(i + 1)], 
    ...
    dx, grads["W"+str(self.num_layers)], grads["b"+str(self.num_layers)] = affine_backward(dx_soft, cache)
    grads["W"+str(self.num_layers)] += self.reg * self.params["W"+str(self.num_layers)]
    for i in range(self.num_layers - 1, 0, -1):
        if self.use_dropout:
            dx = dropout_backward(dx, drop_caches[i - 1])
        if self.use_batchnorm:
            dx, grads["W" + str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_batch_relu_backward(dx, caches[i - 1])
        else:
