...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_mu = x - mu
    x_mu_sq = x_mu ** 2
    var = np.mean(x_mu_sq, axis = 0)
    var_sqrt = np.sqrt(var + eps)
    var_sqrt_invert = 1 / var_sqrt
    x_hat = x_mu * var_sqrt_invert
    out = x_hat * gamma + beta
    cache = (x_hat, gamma, beta, var_sqrt_invert, var_sqrt, var, x_mu_sq, x_mu, mu)
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x_hat, gamma, beta, var_sqrt_invert, var_sqrt, var, x_mu_sq, x_mu, mu = cache
  dgamma = np.sum(dout * x_hat, axis = 0) 
  dbeta = np.sum(dout, axis = 0)
  dx_hat = dout * gamma
  dx_mu_1 = dx_hat * var_sqrt_invert
  dvar_sqrt_invert = np.sum(dx_hat * x_mu, axis = 0)
  dvar_sqrt = -1./(var_sqrt ** 2) * dvar_sqrt_invert
  dvar = 0.5 / var_sqrt * dvar_sqrt
  dx_mu_sq = 1.0/N * np.ones((N,D)) * dvar
  dx_mu_2 = 2 *x_mu * dx_mu_sq
...
def batchnorm_backward_alt(dout, cache):
  ...
  dbeta = np.sum(dout, axis = 0)
  dx_hat = dout * gamma
  dvar = np.sum(dx_hat * x_mu, axis = 0) * (-0.5) * (var_sqrt_invert ** 3)
  dx = 1./var_sqrt * (dx_hat - np.mean(dx_hat, axis = 0)) + 2 * dvar / N * (x_mu - np.mean(x_mu, axis = 0))  
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
